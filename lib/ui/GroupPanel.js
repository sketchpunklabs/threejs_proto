...
/* EXAMPLE
<group-panel label="Controls" float close style="position:fixed; left:calc(100vw - 210px); top:10px; width:200px;">
...
export default class GroupPanel extends HTMLElement{
    // #region MAIN
    static observedAttributes = [ 'close', 'label', 'float' ];
    ...
    constructor(){
        ...
        <style>
            ...
            :host > header > span{ flex:1 1 auto; display:block; }
            :host > header > div{ margin-right:2px; }
            :host > header i{ cursor:pointer; display:inline-block; width:20px; text-align:right; }

            :host([float]) > header > span{ cursor:grab; }
            :host(:not([float])) > header > span{ cursor:pointer; }
    ...
    attributeChangedCallback( name, oldValue, newValue ){
        ...
        switch( name ){
            case 'label': this.header.textContent = newValue; break;
        }
    }
    // #endregion
    ...
    onPointerDown = e=>{
        ...
        if( this.isFloat ) this.header.style.cursor = 'grabbing';
    };
    ...
    onPointerUp = e=>{
        ...
        if( this.isFloat ) this.header.style.cursor = 'grab';
    };
    // #endregion
}
