...
export class TrackVec3{
    // #region MAIN
    jointName  = '';            // Joint name in the skeleton
    jointIndex = -1;            // Joint index in skeleton this track will animate
    ...
    atIndex( i, out=[0,0,0] ){
        const ii = ( i < 0 )? (( this.values.length / 3 ) + i) * 3 : i * 3;
        out[ 0 ] = this.values[ ii + 0 ];
        out[ 1 ] = this.values[ ii + 1 ];
        out[ 2 ] = this.values[ ii + 2 ];
        return out;
    }
    // #endregion
