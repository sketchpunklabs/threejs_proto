...
performance.now()
returns a floating-point number representing milliseconds (with microsecond precision) elapsed since the browser context
*/
...
export default class FixedFrame{
    ...
    #lastTimeStamp = 0;     // DOMHighResTimeStamp from requestAnimationFrame, can generate similar data with performance.now()
    tStep          = 0;     // Normalize Time till next Time Stamp
    frame          = 0;
    onFixedFrame   = null;

    constructor( fps=50, fn=null ){
        this.#timeStep      = 1000 / fps;           // how many milliseconds per frame
        this.#maxTimeStep   = this.#timeStep * 5;
        this.onFixedFrame   = fn;
    }

    set initTimeStamp( v ){ this.#lastTimeStamp = v; }
    ...
    update( ts ){
        ...
        while( this.#accumulate >= this.#timeStep ){
            // console.log( 'ep', this.#accumulate, this.#timeStep );
            if( steps === 0 ) this.frame = ( this.frame + 1 ) % Number.MAX_SAFE_INTEGER;

            if( this.onFixedFrame ) this.onFixedFrame( deltaTime, ts );
