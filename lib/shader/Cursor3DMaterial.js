...
export default class Cursor3DMaterial extends THREE.RawShaderMaterial{
    static createMesh( props={} ){
        const opt = Object.assign( { size:2, w:0, h:0, y:0, factor:0.15, scale:0, }, props );
        const geo = new THREE.PlaneGeometry( opt.w || opt.size, opt.h || opt.size );
        geo.rotateX( -Math.PI * 0.5 );

        const mat  = new Cursor3DMaterial( props );
    ...
    constructor( props={} ){
        super();
        const opt = Object.assign( { factor:0.15, color:'#ffffff' }, props );

        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        this.name           = 'Cursor3DMaterial';
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        this.uniforms = {
            factor  : { type:'float', value: opt.factor },
        ...
        void main(){
            vec3 pos  = ( factor < 0.0001 )? position : position * ( length( cameraPosition - modelMatrix[3].xyz ) * factor );
        ...
        void main(){
            ...
            outColor = vec4( color * c, alpha );
        }`;
    }

    setFactor( x ){
        this.uniforms.factor.value = x;
        return this;
    }
}
