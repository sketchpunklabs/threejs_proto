...
// Test Input : https://hardwaretester.com/gamepad
...
export default class GamepadInput{
    ...
    getPad( idx=0 ){ return this.items[ idx ]; }
    // #endregion
...
class InputProfile{
    ...
    isButtonDown( k ){
        const a = this.mapButtons.get( k );
        return ( a )? a.isDown : false;
    }
}
...
class Axes{
    ...
    update( pad, deadZone ){
        ...
        if( this._xIdx !== -1 ){
            ...
            this.x = ( Math.abs( x ) > deadZone )
                ? ( x - deadZone ) / ( 1 - deadZone )
                : 0;
        }

        if( this._yIdx !== -1 ){
            y = pad.axes[ this._yIdx ];
            this.y = ( Math.abs( y ) > deadZone )
                ? ( y - deadZone ) / ( 1 - deadZone )
                : 0;
        }
...
class AxesSingle{
    ...
    update( pad, deadZone ){
        const v = pad.axes[ this._idx ];
        this.value = ( Math.abs( v ) > deadZone )
                ? ( v - deadZone ) / ( 1 - deadZone )
                : 0;
    ...
    // getDeadZoneT( dz ){
    //     const av = Math.abs( this.value );
    //     if( av <= dz ) return 0;
    //     return ( av - dz ) / ( 1 - dz ) * Math.sign( this.value );
    // }
}
