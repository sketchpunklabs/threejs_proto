...
export function useDarkScene( tjs, props={} ){
    ...
    // Renderer
    tjs.renderer.setClearColor( 0x303035, 1 );
    return tjs;
};
...
/*
Cap DPR by pixel budget
const maxPix = 1_650_000
const dpr    = Math.min(
...
// if you render with high DPI at native resolution, you can disable MSAA,
//
// 2× DPI + no MSAA + a light TAA or blur pass is practically free image quality
*/
...
export default function useThreeWebGL2( props ){
    ...
    if( !props.canvasId ) document.body.appendChild( renderer.domElement );
    ...
    const camCtrl = props.camCtrl ? new OrbitControls( camera, renderer.domElement ) : null;

    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // METHODS
    let self;   // Need to declare before methods for it to be useable

    const render = ( onPreRender=null, onPostRender=null, timeStamp ) =>{
        const deltaTime   = clock.getDelta();
        const ellapseTime = clock.getElapsedTime();

        if( onPreRender )  onPreRender( deltaTime, ellapseTime, timeStamp );
        renderer.render( scene, self.camera );
        if( onPostRender ) onPostRender( deltaTime, ellapseTime, timeStamp );
        return self;
    };
    ...
    const createRenderLoop = ( fnPreRender=null, fnPostRender=null )=>{
        ...
        const rLoop = {
            ...
            start       : ()=>rLoop.onRender( 0 ),
            onRender    : ( ts )=>{ // requestAnimationFrame passes DOMHighResTimeStamp
                render( fnPreRender, fnPostRender, ts );
    ...
    const sphericalLook = ( lon, lat, radius, target=null )=>{
        ...
        if( target && self.camCtrl ){
            self.camCtrl.target.fromArray( target );
            pos[0] += target[0];
            pos[1] += target[1];
            pos[2] += target[2];
        }
        ...
        if( self.camCtrl ) camCtrl.update();
        return self;
    };

    const resize = ( w=0, h=0 )=>{
        const W = w || canvas.parentNode.clientWidth; //window.innerWidth;
        const H = h || canvas.parentNode.clientHeight; //window.innerHeight;
